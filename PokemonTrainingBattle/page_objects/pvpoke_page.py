...
class PvPokePage(AbstractPage):
    ...
    search_text_field = (
        "text=Random Swap Select a Pokemon AbomasnowAbomasnow (Mega)Abomasnow "
        '(Shadow)AbraAbra >> [placeholder="Search name"]'
    )
    opponent_search_text_field = '[placeholder="Search name"] >> nth=1'
    pick_pokemon_dropdown = (
        "text=Random Swap Select a Pokemon AbomasnowAbomasnow (Mega)Abomasnow " "(Shadow)AbraAbra >> select >> nth=0"
    )
    pokemon_selection_dropdown = (
        "text=Random Swap Select a Pokemon AbomasnowAbomasnow " "(Mega)Abomasnow (Shadow)AbraAbra >> select >> nth=0"
    )
    ...
    def select_difficulty(self):
        difficulty_list = ["Novice", "Rival", "Elite", "Champion"]
    ...
    def pull_stats(self):
        ...
        pokemon_names = (
            self.page.query_selector_all('//table[contains(@class, "stats-table")]')[3]
            .query_selector_all("//tr")[0]
            .text_content()
            .split()[0:2]
        )
        pokemon_battle_ratings = (
            self.page.query_selector_all('//table[contains(@class, "stats-table")]')[3]
            .query_selector_all("//tr")[1]
            .inner_text()
            .split()[2:4]
        )
        pokemon_total_damage = (
            self.page.query_selector_all('//table[contains(@class, "stats-table")]')[3]
            .query_selector_all("//tr")[2]
            .inner_text()
            .split()[2:4]
        )
        pokemon_fast_move_damage = (
            self.page.query_selector_all('//table[contains(@class, "stats-table")]')[3]
            .query_selector_all("//tr")[3]
            .inner_text()
            .split("\t")[1:3]
        )
        pokemon_charged_move_damage = (
            self.page.query_selector_all('//table[contains(@class, "stats-table")]')[3]
            .query_selector_all("//tr")[4]
            .inner_text()
            .split("\t")[1:3]
        )
        pokemon_turns_to_first_charged_move = (
            self.page.query_selector_all('//table[contains(@class, "stats-table")]')[3]
            .query_selector_all("//tr")[6]
            .inner_text()
            .split("\t")[1:3]
        )
        pokemon_energy_gained = (
            self.page.query_selector_all('//table[contains(@class, "stats-table")]')[3]
            .query_selector_all("//tr")[7]
            .inner_text()
            .split()[2:4]
        )
        pokemon_energy_used = (
            self.page.query_selector_all('//table[contains(@class, "stats-table")]')[3]
            .query_selector_all("//tr")[8]
            .inner_text()
            .split()[2:4]
        )
        pokemon_energy_remaining = (
            self.page.query_selector_all('//table[contains(@class, "stats-table")]')[3]
            .query_selector_all("//tr")[9]
            .inner_text()
            .split()[2:4]
        )
        pokemon_stats = {}
